        ...
        {props.data.list.map((item, idx) => {
          return (
            <Col xs={24} md={24} lg={12} key={idx}>
              <motion.div
                className="h-full rounded-lg border-2 border-solid border-[#E7E1D9] bg-[#FDFAF6] p-[25px]"
                initial="hide"
                whileInView="show"
                viewport={{ once: true }}
                exit="hide"
              ...
              >
                {item.map((i, index) => {
                  return (
                    <Row
                      className="text-20px text-left font-bold text-brown-1000 mb-12"
                      key={index}
                    >
                      <Col span={2}>
                        <div className="tex-left justify-left mb-8 flex">
                          <Image src={flowerIcon} alt="block" />
                        </div>
                      </Col>
                      <Col span={22}>
                        <div className="475px:text-16px">{i.label}</div>
                      </Col>
                    </Row>
                  );
                })}
